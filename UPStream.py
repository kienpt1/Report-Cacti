...
def read_json(json_path):
    ...
    for pop in ["HKG", "SGP", "JPN"]:
        ...
        for device_name, entries in pop_data.items():
...
def aggregate_data(results, rrd_files):
    ...
    for idx, (rrd_file, data) in enumerate(results):
        ...
        if device not in ring_data:
            ...
            # Merge additional data (for traffic_out)
            ring_data[device]["df"] = pd.merge(
                ring_data[device]["df"],
                df,
                on="timestamp",
                how="outer",
            ...
            ring_data[device]["df"]["traffic_in"] = ring_data[device]["df"][["traffic_in", "traffic_in_extra"]].max(axis=1)
            ring_data[device]["df"]["traffic_out"] = ring_data[device]["df"][["traffic_out", "traffic_out_extra"]].max(axis=1)
            ring_data[device]["df"].drop(columns=["traffic_in_extra", "traffic_out_extra"], inplace=True)
    return ring_data
